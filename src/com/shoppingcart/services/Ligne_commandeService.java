...
import com.shoppingcart.utils.DatabaseConnection;
...
public class Ligne_commandeService {
    ...
    public List<Ligne_commande> getAll() {
        ...
        try {
            preparedStatement = connection.prepareStatement("SELECT * FROM `ligne_commande` AS l RIGHT JOIN `products` AS p ON l.produit_id = p.id RIGHT JOIN `commande` AS c ON l.commande_id = c.id WHERE l.produit_id = p.id;");
